...
**How I reproduced it:** Since `/songs/<id>/listen` always timestamps with the server's real current time, I set up the precondition directly against the database: gave a friend exactly one listening event and set its `listened_at` to 20 hours before "now," which lands it on the previous calendar day. Calling `get_friends_listening_now` (and, live, `GET /feed/<user_id>/listening-now`) still returned that friend.

**How I found the root cause:** Opened `services/feed_service.py`. `get_friends_listening_now` defines `RECENT_THRESHOLD = timedelta(hours=24)` and builds `cutoff = datetime.now(timezone.utc) - RECENT_THRESHOLD`, then filters `ListeningEvent.listened_at >= cutoff`. There's no reference anywhere to calendar dates, only an elapsed-time window measured from the moment of the request.

**The root cause:** "Listening now" is meant to mean "today," but the filter checks elapsed hours since the request, not whether the event happened on the current calendar day. An event from 8pm yesterday is only 20 hours old at 4pm today, well inside the 24-hour window, so it reads as current even though a user would call it "yesterday." The bug is a mismatch between the feature's intent (today's activity) and what the code actually measures (the last 24 hours, a window that slides with the clock instead of resetting at midnight).

**My fix and side-effect check:** Replaced the rolling cutoff with the start of the current UTC calendar day: `cutoff = now.replace(hour=0, minute=0, second=0, microsecond=0)`, and removed the now-unused `RECENT_THRESHOLD` constant and `timedelta` import. Reran the same setup (friend's only event 20 hours old, dated yesterday); the friend no longer appears in the feed. Checked an event from 2 hours ago on the same calendar day still appears, confirming same-day events aren't excluded. Also checked `get_activity_feed`, which intentionally has no recency filter at all and is unaffected since it doesn't reference `cutoff`. Ran the full test suite: no new failures, same two pre-existing `test_playlists.py` failures tied to Issue 5.

### Issue 3: The same song keeps showing up twice in search

**How I reproduced it:** Earlier, `curl "http://127.0.0.1:5000/songs/search?q=Crown"` against a song seeded with three tags came back with `"count":1`, not duplicated, on this environment's SQLAlchemy version. So instead of relying on the JSON response, I checked what the query produces at the SQL level directly: compiled the exact statement `search_songs` builds and executed it against the raw connection (bypassing the ORM's entity mapping). That returned 3 duplicate rows, one per tag, for the same song. The duplication was real, it just wasn't reaching the HTTP response because of an ORM version quirk that happened to deduplicate full-entity results before returning them.

**How I found the root cause:** Opened `services/search_service.py` and read `search_songs`. It builds `db.session.query(Song).outerjoin(song_tags, Song.id == song_tags.c.song_id).filter(...)`. The filter only checks `Song.title` and `Song.artist`, never anything from `song_tags`. I checked how tags actually get into the response and found `Song.tags` on the model is `db.relationship("Tag", secondary=song_tags, lazy="subquery")`, meaning tags are loaded through a separate query entirely when `to_dict()` runs. That made the join in `search_songs` pointless: it isn't used for filtering and isn't needed for tags to appear in the output, its only effect is multiplying each song's row once per tag it has.

**The root cause:** The query joins `Song` to `song_tags` for no reason connected to what the function actually filters or returns. A song with N tags produces N rows in the joined result set (a classic join fan-out), and every one of those rows maps to the same song. Whether that fan-out becomes a visible duplicate in the API response turned out to depend on ORM behavior that happened to collapse full-entity duplicates back down on this SQLAlchemy version, which is exactly why the bug didn't reproduce through curl even though the underlying query was still wrong. Removing the dead join fixes the defect at its source rather than relying on that ORM behavior to keep masking it.

**My fix and side-effect check:** Removed `.outerjoin(song_tags, Song.id == song_tags.c.song_id)` from `search_songs` and dropped the now-unused `song_tags` import. Reran the raw-SQL check from the reproduction step: the compiled statement now returns exactly 1 row for the three-tag song, confirming the fan-out is gone at the SQL level, not just hidden by the ORM. Confirmed `search_songs("Crown")` still returns the song with its tags list intact (`["rap", "hip-hop", "boom bap"]`), since tag loading comes entirely from the separate `Song.tags` relationship and was never dependent on the join. Ran the full test suite: `tests/test_search.py` still passes, same two pre-existing `test_playlists.py` failures for Issue 5, no new breakage.

### Issue 4: Notified when a friend adds my song to a playlist, but not when they rate it

**How I reproduced it:** Recorded a song's sharer's notification count via `GET /users/<id>/notifications`, then rated that song as a different user via `POST /songs/<id>/rate`, then checked the count again. The rating was created successfully (a real `Rating` row came back in the response), but the sharer's notification count didn't change.

**How I found the root cause:** Both `add_to_playlist` and `rate_song` live in `services/notification_service.py`, so I compared them line by line since the module's docstring says it exists to notify sharers when friends interact with their songs. `add_to_playlist` ends with `if song.shared_by != added_by_user_id: create_notification(...)`. `rate_song` has no equivalent anywhere in its body: it validates the score, loads the song and rater, saves or updates the `Rating` row, commits, and returns. There's no branch that ever touches `create_notification`.
...
**How I reproduced it:** Compared a playlist's actual song count (`len(playlist.songs)`, backed by the `playlist_entries` relationship) against what `GET /playlists/<id>/songs` returns and against `get_playlist_songs` directly. Every seeded playlist has 7 songs but the endpoint reported 6. Adding one more song via `POST /playlists/<id>/songs` and re-checking still came up exactly one short of the true total.

**How I found the root cause:** Opened `services/playlist_service.py` and read `get_playlist_songs`. It builds a query joining `Song` to `playlist_entries`, filtered to the given playlist and ordered by `position`, which is the correct query for "songs in playlist order." The very last line, `return [song.to_dict() for song in songs[:-1]]`, is what caught my attention: after doing the filtering and ordering correctly, it slices the already-correct list before returning it.

**The root cause:** `songs[:-1]` drops the last element of a Python list. Since `songs` is already the fully filtered, correctly position-ordered list of every song in the playlist, this slice always discards whichever song sits in the final position, regardless of how many songs the playlist has or when the discarded one was added. It isn't an off-by-one in the query or the join, the data fetched is entirely correct up to that point; the bug is a single stray slice applied to an already-correct result.

**My fix and side-effect check:** Changed `songs[:-1]` to `songs`, returning the full ordered list. Reran the reproduction steps: all three seeded playlists now report 7 songs, matching their actual contents, and a newly-added song is included immediately. Checked the boundary case directly: built a playlist with exactly one song in an isolated in-memory database. Under the old code this returned an empty list (`[:-1]` on a one-item list is empty); with the fix it correctly returns that one song. Ran the full test suite: all 13 tests pass, including the two `test_playlists.py` cases that had been failing throughout the rest of this work and were tied specifically to this bug.
